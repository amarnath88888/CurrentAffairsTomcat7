...
/**
 * This is a DAO class, for the collection Current Affairs to process Current Affairs related activities.
 * Contains list of DAO methods that are needed to perform the CRUD operations on the Current Affairs Object
 * @author Amar
 *
 */
public class CurrentAffairsDAO implements ICurrentAffairs {

	private final DBCollection businessCollection;

	public CurrentAffairsDAO(DBCollection dbCollection) {
		businessCollection = dbCollection;
	}

	public List<DBObject> getAllNEWSAsList() {
		DBCursor businessCursor = businessCollection.find();
		return businessCursor.toArray();
	}
    /* (non-Javadoc)
     * Add a Current Affairs to the Collection
    ...
    /* (non-Javadoc)
     * Delete an existing Current Affairs record
